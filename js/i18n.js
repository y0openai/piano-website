...
(function () {
  ...
  const T = {
    en: {
      ...
      'index.track.sub': 'The figures below are the actual performance of one running configuration. Past results do not guarantee future returns.',
      ...
      'index.track.disclaimer': 'These are realized past results of a single running configuration (gamma), losses included. Return is the change in the account’s net equity over the measurement period (no deposits or withdrawals), and win rate counts individual hedge legs. Past performance, backtests, and simulations do not guarantee future returns, and you may lose principal. The same tool is provided to every user.',
    ...
    ko: {
      ...
      'index.track.sub': '아래 숫자는 실제로 운용 중인 한 설정(라흐마니노프)의 성과입니다. 과거 실적은 미래 수익을 보장하지 않습니다.',
      ...
      'index.track.disclaimer': '손실을 포함한, 운용 중인 한 설정(감마)의 실현 과거 실적입니다. 수익률은 측정 기간 중 계좌 순자산의 변화(입출금 없음 기준)이며, 승률은 헷지 개별 레그 기준입니다. 과거 성과·백테스트·시뮬레이션은 미래 수익을 보장하지 않으며, 원금 손실이 발생할 수 있습니다. 모든 사용자에게 동일한 도구가 제공됩니다.',
